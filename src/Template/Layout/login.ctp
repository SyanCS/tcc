...
<body class="hold-transition login-page" style="background:rgb(54,127,169)">
<div class="login-box">
  <div class="login-logo">
    <span style="color:white"> <strong>TCC</strong> </span>
  </div>
  <!-- /.login-logo -->
...
<?php echo $this->fetch('content'); ?>
    ...
    if (Configure::read('Theme.login.show_remember')) {
        ?>
        <a href="<?php echo $this->Url->build(array('controller' => 'users', 'action' => 'forgot', 'home'));?>"><?php echo __('I forgot my password') ?></a><br>
        <?php
    }
    if (Configure::read('Theme.login.show_register')) {
        ?>
        <a href="<?php echo $this->Url->build(array('controller' => 'users', 'action' => 'register', 'home'));?>" class="text-center"><?php echo __('Register a new membership') ?></a>
        <?php
    }
    ?>
